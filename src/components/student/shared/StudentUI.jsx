...
export function StatusBadge({ status }) {
  ...
  if (['paid', 'success', 'completed', 'present', 'resolved', 'approved', 'issued', 'pass', 'active', 'on track'].includes(key)) {
    classes = 'bg-[#00A896]/15 text-[#005F6B]'
  } else if (['pending', 'due', 'upcoming', 'open', 'live soon'].includes(key)) {
    classes = 'bg-amber-100 text-amber-700'
  } else if (['late', 'absent', 'failed', 'fail', 'overdue', 'rejected'].includes(key)) {
    classes = 'bg-rose-100 text-rose-700'
  } else if (['leave', 'partial', 'scheduled', 'refunded', 'cancelled'].includes(key)) {
