...
export function StatusBadge({ status }) {
  ...
  if (['paid', 'success', 'completed', 'present', 'resolved', 'approved', 'issued', 'pass', 'active', 'on track', 'published', 'correct'].includes(key)) {
    classes = 'bg-[#00A896]/15 text-[#005F6B]'
  } else if (['pending', 'due', 'upcoming', 'open', 'live soon', 'live', 'in progress', 'draft', 're-exam allotted', 'on leave'].includes(key)) {
    classes = 'bg-amber-100 text-amber-700'
  } else if (['late', 'absent', 'failed', 'fail', 'overdue', 'rejected', 'dropped', 'suspended'].includes(key)) {
